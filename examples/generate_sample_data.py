...
# 1. Peeling chain realist, cu sume explicite per output (corect, nu split egal)
current = addr("PEEL", 0)
amount = 50.0
for i in range(6):
...
# 2. Fan-out: o adresa trimite catre 14 adrese noi (posibil smurfing)
...
# 3. Fan-in: 12 adrese trimit catre o singura adresa de agregare
...
# 4. Rapid pass-through: adresa primeste si retrimite aproape tot in <1h
rp_in = addr("RPIN", 0)
rp_mid = addr("RPMID", 0)
rp_out = addr("RPOUT", 0)
t0 = ts
...
# 5. Common-input-ownership: o tranzactie cu 3 inputuri comune (acelasi proprietar probabil)
...
print(f"Generat {len(txs)} tranzactii sintetice.")

with open("data/sample/sample_blacklist.txt", "w", encoding="utf-8") as f:
    f.write("CASHOUT0003\nCASHOUT0005\n")

print("Blacklist sample generat.")
